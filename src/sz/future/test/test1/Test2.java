...
//		System.out.println(path + " : " + size);
		...
		for (int row = 0; row < csvList.size(); row++) {
			Global.updateTimeArray[row] = csvList.get(row)[1];
...
//		}
		...
		double highestPriceOfDay = 0d;
		double lowestPriceOfDay = 100000d;
...
//					System.out.println("大于"+Global.period+"天最高价"+ highestPrice);
				...
				if(highestPriceOfDay < Global.lastPriceArray[i]){
					...
					lowestPriceOfDay = Global.lastPriceArray[i];
				}
			} else {
...
//				boolean closeFlag4 = false ;
				...
				if(Global.bs){//持有多头头寸
					...
					if(StatisticsUtil.daysBetween(Global.openPositionDate, Global.tradingDay) > 4){
						...
						if(Global.highestProfit > 10){
							if((Global.lastPriceArray[i] - Global.positionPrice) < Global.highestProfit * 0.4){
								closeFlag3 = true;
							}
						}
...
//						if(closeFlag4)System.out.println(++Global.closePositionCount3 + " flag4..............."+Global.lastPriceArray[i]);
					...
					if(StatisticsUtil.daysBetween(Global.openPositionDate, Global.tradingDay) > 4){
						...
						if(Global.highestProfit > 10){
							if((Global.positionPrice - Global.lastPriceArray[i]) < Global.highestProfit * 0.4){
								closeFlag3 = true;
							}
						}
