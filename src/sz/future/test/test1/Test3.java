...
//	private static List<MdDay> dayList = new ArrayList<MdDay>();
	...
	public static void main(String[] args) throws InterruptedException {
		for (int i = 0; i < Global.test_instrument_id_array.length; i++) {
			...
			Global.highestMd = dao.loadDayData2(Global.test_instrument_id);
			Global.lowestMd = dao.loadDayData3(Global.test_instrument_id);
			queryMd();
			print();
		}
	}
...
////			System.out.println("date  ---------- "+array[i-1]);
//			if(array[i-1].equals(Global.tradingDay) && (i+1)<=(array.length-1)){
//				date = (Date)array[i+1];
//			}
//		}
//		double perHighestPrice = 0d;
//		double perLowestPrice = 0d;
//		if(date != null){
//			perHighestPrice = Global.highestMd.get(date);
//			perLowestPrice = Global.lowestMd.get(date);
//		}
...
//		double highestPrice = highestPpriceArray.get(highestPpriceArray.size()-1);//Global.period天之内最高收盘价
		...
		double beforMa4 = StatisticsUtil.getBeforMA(1, 4);//一天前的MA5
		double beforMa9 = StatisticsUtil.getBeforMA(1, 9);//一天前的MA10
		double beforMa18 = StatisticsUtil.getBeforMA(1, 18);//一天前的MA10
		double beforMa10 = StatisticsUtil.getBeforMA(1, 10);//一天前的MA10
		double beforMa20 = StatisticsUtil.getBeforMA(1, 20);//一天前的MA10
		double beforMa50 = StatisticsUtil.getBeforMA(1, 50);//一天前的MA10
		
		for (int i = 100; i < Global.lastPriceArray.length; i=i+300) {
...
//				double b = Global.lastPriceArray[i-100];
...
//				}
	...
	private static void print() {
		...
		System.out.println("符合条件3平仓次数：" + Global.closePositionCount3);
